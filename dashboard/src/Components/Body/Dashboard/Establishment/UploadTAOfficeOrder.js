...
const UploadTAOfficeOrder = ({ permission }) => {
  ...
  useEffect(() => {
    fetch("/api/ta/pending-office-orders")
      .then((data) => data.json())
      .then((data) => {
        console.log(data);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <TADialogBox request_id={id} showCommentSection={false} />
          <DialogActions>
        ...
        >
          <UploadDialogBox request_id={id} api={"/api/ta/upload-office-order"} />
          <DialogActions>
...
export default UploadTAOfficeOrder